...
class TestOdoobot(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.kodabot = cls.env.ref("base.partner_root")
        ...
        cls.kodabot_ping_body = '<a href="http://koda.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@OdooBot</a>' % (cls.kodabot.id, cls.kodabot.id)
    ...
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_kodabot()
        kodabot = self.env.ref("base.partner_root")
        kodabot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', kodabot.id)])
        self.assertEqual(len(kodabot_in_fetch_listeners), 1, 'kodabot should appear only once in channel_fetch_listeners')

    @mute_logger('koda.addons.mail.models.mail_mail')
    def test_kodabot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.kodabot_ping_body, 'partner_ids': [self.kodabot.id, self.user_admin.partner_id.id]})
        ...
        self.assertNotIn(self.kodabot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last kodabot message instead
            sender=self.kodabot,
            answer=("@OdooBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.kodabot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.kodabot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('koda.addons.mail.models.mail_mail')
    def test_kodabot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @kodabot right now", 'partner_ids': []})
        self.assertNextMessage(

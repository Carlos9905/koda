...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/koda" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/koda/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/koda/* /home/pi/koda/
rm -r /home/pi/koda/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service koda restart) &

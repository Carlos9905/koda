...
from koda.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if koda.tools.config['db_host']:
        env['PGHOST'] = koda.tools.config['db_host']
    if koda.tools.config['db_port']:
        env['PGPORT'] = str(koda.tools.config['db_port'])
    if koda.tools.config['db_user']:
        env['PGUSER'] = koda.tools.config['db_user']
    if koda.tools.config['db_password']:
        env['PGPASSWORD'] = koda.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = koda.addons.__path__ + [root_path]
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = koda.fields.Datetime.from_string(value)
            value = koda.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = koda.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = koda.fields.Datetime.from_string(value)
    else:
        timestamp = value

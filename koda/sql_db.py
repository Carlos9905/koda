...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # koda Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri):
    ...
    :rtype: (str, dict)
    """
    if 'koda_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['koda_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
...
def db_connect(to, allow_uri=False):
    ...
    if _Pool is None:
        _Pool = ConnectionPool(int(
            koda.evented and tools.config['db_maxconn_gevent']

...
r"""\
koda HTTP layer / WSGI application
...
Application.__call__
  ...
  request and itself in an koda http request. The koda http request is
...
koda URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.koda.com/documentation/17.0/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through koda via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    """
    try:
        dbs = koda.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, koda will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': koda.service.common.dispatch,
        'db': koda.service.db.dispatch,
        'object': koda.service.model.dispatch,
    }
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are koda addons.
        """
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, login=None, password=None):
        ...
        with registry.cursor() as cr:
            env = koda.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            ...
            request.env = koda.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~koda.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> koda_ip = socket.gethostbyname('koda.com')
        >>> GeoIP(koda_ip).country.iso_code
        'FR'
    """
...
class Request:
    ...
    def _serve_db(self):
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            self.env = koda.api.Environment(cr, self.session.uid, self.session.context)
            threading.current_thread().uid = self.env.uid
            try:
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "koda Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in koda.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + koda.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = koda.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if koda.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):

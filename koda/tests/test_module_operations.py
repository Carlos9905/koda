...
import koda
from koda.tools import config, topological_sort, unique
from koda.netsvc import init_logger
...
def install(db_name, module_id, module_name):
    with koda.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of koda modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--data-dir", "-D", dest="data_dir", type=str,
        help="Directory where to store koda data"
    )
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra koda modules")
...
def test_cycle(args):
    ...
    with koda.registry(args.database).cursor() as cr:
        env = koda.api.Environment(cr, koda.SUPERUSER_ID, {})
...
def test_standalone(args):
    ...
    registry = koda.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        koda.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        koda.tools.config['addons_path'] = ','.join([args.addons_path, koda.tools.config['addons_path']])
        if args.data_dir:
            koda.tools.config['data_dir'] = args.data_dir
        koda.modules.module.initialize_sys_path()

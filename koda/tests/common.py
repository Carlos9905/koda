...
from koda.sql_db import BaseCursor, Cursor
from koda.tools import float_compare, single_email_re, profiler, lower_logging, SQL
from koda.tools.misc import find_in_path, mute_logger
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 17.0: koda.tests.common.Form is deprecated, use koda.tests.Form",
...
ADMIN_USER_ID = koda.SUPERUSER_ID
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding koda module name.
    """
    def register(func):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for koda-specific code. This class is abstract and
    ...
    def run(self, result):
        ...
        if getattr(testMethod, '_retry', True) and getattr(self, '_retry', True):
            tests_run_count = int(os.environ.get('koda_TEST_FAILURE_RETRIES', 0)) + 1
        else:
            tests_run_count = 1
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(koda.modules.registry._logger.name)
    ...
    def _gc_filestore(cls):
        ...
        with koda.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, koda.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, koda.SUPERUSER_ID, {})
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot'):
    ...
    screenshots_dir = pathlib.Path(koda.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_class, headless=True):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_koda')

        otc = koda.tools.config
...
which leads to stray network requests and inconsistencies."""))
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way koda widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{koda.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        if request_threads:
            ...
            koda.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        koda.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            koda.http.root.session_store.delete(self.session)

        self.session = session = koda.http.root.session_store.new()
        session.update(koda.http.get_default_session(), db=get_db_name())
        session.context['lang'] = koda.http.DEFAULT_LANG
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{koda.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "koda.startTour('%s', %s)" % (tour_name, json.dumps(options)))
        ready = kwargs.pop('ready', "koda.isTourReady('%s')" % tour_name)

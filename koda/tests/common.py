...
from koda.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from koda.service import security
from koda.sql_db import BaseCursor, Cursor
from koda.tools import float_compare, single_email_re, profiler, lower_logging
from koda.tools.misc import find_in_path
    ...
    def _process_fvg(self, model, fvg, level=2):
        ...
        * pre-processed onchanges list
        ...
        eval_context = {
            ...
            "datetime": datetime,
            "context_today": lambda: koda.fields.Date.context_today(self._env.user),
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = koda.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = koda.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return koda.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return koda.fields.Date.to_string(value)
    return value

...
import koda
from koda.modules.db import FunctionStatus
from koda.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from koda.sql_db import TestCursor
from koda.tools import (config, existing_tables, lazy_classproperty,
                        lazy_property, sql, Collector, OrderedSet)
from koda.tools.func import locked
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                koda.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                koda.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._assertion_report = koda.tests.result.OdooTestResult()
        ...
        self._db = koda.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = koda.modules.db.has_unaccent(cr)
            self.has_trigram = koda.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = koda.api.Environment(cr, SUPERUSER_ID, context)

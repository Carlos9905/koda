...
def is_initialized(cr):
    ...
    """
    return koda.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    with koda.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in koda.modules.get_modules():
        mod_path = koda.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = koda.modules.get_manifest(i)

...
import koda.modules.graph
import koda.modules.migration
...
def force_demo(env):
    ...
    """
    graph = koda.modules.graph.Graph()
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = koda.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = koda.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = koda.sql_db.sql_counter
        ...
        if needs_update:
            ...
            overwrite = koda.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            loader = koda.tests.loader
            suite = loader.make_suite([module_name], 'at_install')
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), koda.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = koda.sql_db.sql_counter - tests_q0
        ...
        extra_queries = koda.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   koda.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not koda.modules.db.is_initialized(cr):
        ...
        if update_module and koda.tools.table_exists(cr, 'ir_model_fields'):
        ...
        migrations = koda.modules.migration.MigrationManager(cr, graph)
        for package in graph:
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                registry = koda.modules.registry.Registry.new(

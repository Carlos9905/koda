...
import koda
import koda.tools as tools
import koda.release as release
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in koda.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = koda.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(koda.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'koda.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in koda.addons.__path__:

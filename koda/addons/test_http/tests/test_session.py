...
class TestHttpSession(TestHttpBase):
    ...
    def test_session2_geoip(self):
        real_save = koda.http.root.session_store.save
        with patch.object(koda.http.root.geoip_resolver, 'resolve') as mock_resolve,\
             patch.object(koda.http.root.session_store, 'save') as mock_save:

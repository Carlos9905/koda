...
from koda.exceptions import AccessDenied, AccessError, UserError, ValidationError
from koda.http import request, DEFAULT_LANG
from koda.osv import expression
from koda.service.db import check_super
from koda.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by koda (updates, module installation, ...)'))
        self.env.registry.clear_cache()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this koda portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your koda is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "koda in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.koda.com/documentation/17.0/administration/install/deploy.html#https for details.",
                    source
                )

    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = koda.fields.Datetime.now()
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(koda.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(koda.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:

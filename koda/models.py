...
class BaseModel(metaclass=MetaModel):
    """Base class for koda models.

    koda models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    _module = None              #: the model's module (in the koda sense)
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], koda.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], koda.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from koda.fields import Char, Integer
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~koda.api.depends` and
        context-dependent using :attr:`~koda.api.depends_context`.
        """
        if self._rec_name:
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = koda.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else koda.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = koda.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else koda.fields.Date.to_date(fill_to)
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted koda models.

    koda models are created by inheriting from this class::

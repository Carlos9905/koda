...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = koda.tools.config.parser
        ...
        opt = koda.tools.config.parse_config(args)

        dbname = koda.tools.config['db_name']
        if not dbname:
        ...
        try:
            with koda.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = koda.modules.neutralize.get_installed_modules(cursor)
                    queries = koda.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    koda.modules.neutralize.neutralize_database(cursor)

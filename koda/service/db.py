...
import koda.sql_db
import koda.tools
from koda.sql_db import db_connect
from koda.release import version_info
from koda.tools import find_pg_tool, exec_pg_environ
...
def check_super(passwd):
    if passwd and koda.tools.config.verify_admin_password(passwd):
        return True
...
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            koda.modules.db.initialize(cr)
            koda.tools.config['load_language'] = lang
            cr.commit()

        registry = koda.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = koda.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = koda.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = koda.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = koda.tools.config['db_template']
    ...
    try:
        db = koda.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if koda.tools.config['unaccent']:
                ...
                # But in the case of koda, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    koda.sql_db.close_db(db_original_name)
    db = koda.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = koda.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = koda.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            koda.modules.neutralize.neutralize_database(cr)

    from_fs = koda.tools.config.filestore(db_original_name)
    to_fs = koda.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    koda.modules.registry.Registry.delete(db_name)
    koda.sql_db.close_db(db_name)
    ...
    fs = koda.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'koda_dump': '1',
        'db_name': cr.dbname,
        'version': koda.release.version,
        'version_info': koda.release.version_info,
        'major_version': koda.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = koda.tools.config.filestore(db_name)
            ...
            if stream:
                koda.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                koda.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = koda.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = koda.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    koda.modules.registry.Registry.delete(old_name)
    koda.sql_db.close_db(old_name)
    ...
    old_fs = koda.tools.config.filestore(old_name)
    new_fs = koda.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    koda.tools.config.set_admin_password(new_password)
    koda.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        koda.tools.config['update']['base'] = True
        koda.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
def list_dbs(force=False):
    if not koda.tools.config['list_db'] and not force:
        raise koda.exceptions.AccessDenied()

    if not koda.tools.config['dbfilter'] and koda.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, koda will not
        ...
        res = sorted(db.strip() for db in koda.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [koda.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of koda
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if koda.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        koda.sql_db.close_db(database_name)
...
def exp_list_lang():
    return koda.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(koda.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
